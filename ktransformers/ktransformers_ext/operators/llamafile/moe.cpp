...
void MOE::backward_one(int k, const uint64_t* expert_ids, const float* weights, const void* output_grad, void* input_grad, Backend* backend, const MoEForwardCache* fwd_cache) {
    ...
    backend->do_work_stealing_job(nth, nullptr, [&](int task_id) {
        ...
        for (int expert_idx = 0; expert_idx < k; expert_idx++) {
            ...
            for (int i = ith * config_.stride; i < (ith + 1) * config_.stride; i++) {
				float bef_acc = s_input_grad_fp32_[i];
                s_input_grad_fp32_[i] += s_gate_input_grad_[expert_idx][i] + s_up_input_grad_[expert_idx][i];
				
				if (fast_isnan(s_gate_input_grad_[expert_idx][i]) || fast_isnan(s_up_input_grad_[expert_idx][i])) {
					std:: cout << "i: " << i << "; expert_idx: " << expert_idx << "; gate_input_grad_ptr: " << gate_input_grad_ptr[i - ith * config_.stride] << "; up_input_grad_ptr: " << up_input_grad_ptr[i - ith * config_.stride] << "; bef_s_input_grad: " << bef_acc << "; aft_s_input_grad: " << s_input_grad_fp32_[i] << std::endl;
				}
            }
        }
		std::cout.flush();
		...
		// 	// 	if (fast_isnan(gate_input_grad_ptr[i - ith * config_.stride]) || fast_isnan(up_input_grad_ptr[i - ith * config_.stride])) {
		// 	// 		std:: cout << "i: " << i << "; expert_idx: " << expert_idx << "; gate_input_grad_ptr: " << gate_input_grad_ptr[i - ith * config_.stride] << "; up_input_grad_ptr: " << up_input_grad_ptr[i - ith * config_.stride] << "; bef_s_input_grad: " << bef_acc << "; aft_s_input_grad: " << s_input_grad_fp32_[i] << std::endl;
		// 	// 		// continue;
		// 	// 	}
		// 	// }
...
void MOE::backward(int qlen, int k, const uint64_t* expert_ids, const float* weights,
	...
	std::cout << "GO INTO BACKWARD!!!" << std::endl;
    for (int i = 0; i < qlen; i++) {
        backward_one(k,
                     expert_ids + i * k,
                     weights + i * k,
